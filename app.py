...
class DataManuplication:
      ...
      def InsertData(self): #insert data on the DB
          self.getColumns = self.Merge_data_set2.columns.tolist()
          self.cols = ','.join([str(i) for i in self.getColumns ])

          #insert data into table
          for i,row in self.Merge_data_set2.iterrows():
              sql = 'INSERT INTO movie_catalogue.movies(' + self.cols + ') VALUES ('+'%s,' *(len(row)-1)+'%s)'
